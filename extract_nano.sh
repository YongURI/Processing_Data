...
#mseeddir=/run/media/yuy/5C2C52412C5215FC/201708_HN_serve
mseeddir=/run/media/yuy/BE72E2A572E2619F/HN_I/CEA/CEA_201411-201507/NANO
if [ ! -e $mseeddir ];then 
mkdir $mseeddir
fi
...
sacdir=/data/HN_sac/HN_I_CEA/with_resp_nano
if [ ! -e $sacdir ];then
mkdir $sacdir
fi

tmpdir=/data/tmp_nano
if [ ! -e $tmpdir ];then
mkdir $tmpdir
fi
...
#catlst=/home/yuy/Project/HUANAN/ST_info/USGS_HN.Regional_201601_201812_M5.5_D20_150.csv
...
cat $stlst | grep "CEA" | grep "Taurus" |while read stline;do 
  echo $stline
  ...
  stele=`echo $stlline | awk '{print $5}'`
  #stele=1
...
###########################################################################################     
    if [ $fjulday -eq $tjulday ];then
      ...
      do
        ...
        filelst01="$filelst01"" "./$fyear$fmonth/*HHZ*_"$fyear""$fmonth""$fday"_"$hourdir"*.miniseed
        filelst02="$filelst02"" "./$fyear$fmonth/*HHN*_"$fyear""$fmonth""$fday"_"$hourdir"*.miniseed
        filelst03="$filelst03"" "./$fyear$fmonth/*HHE*_"$fyear""$fmonth""$fday"_"$hourdir"*.miniseed
        ((i=$i+1))
      done
    else
      i=`echo $fhour | bc -l`
	...
	filelst01="$filelst01"" "./$fyear$fmonth/*HHZ*_"$fyear""$fmonth""$fday"_"$hourdir"*.miniseed
      ...
      do
        hourdir=`echo $i | awk '{printf("%02d\n",$1)}'`
	filelst01="$filelst01"" "./$tyear$tmonth/*HHZ*_"$tyear""$tmonth""$tday"_"$hourdir"*.miniseed
        filelst02="$filelst02"" "./$tyear$tmonth/*HHN*_"$tyear""$tmonth""$tday"_"$hourdir"*.miniseed
        filelst03="$filelst03"" "./$tyear$tmonth/*HHE*_"$tyear""$tmonth""$tday"_"$hourdir"*.miniseed
        ((i=$i+1))
      done
    fi      
#############################################################################################  
...
#    echo $filelst01   >>filelist.lst                  #the mseed file list used to merge
  ...
  mv *.HHZ.*.$year.$julday.$hour$min$sec.SAC $outdir/$stn.$year$julday$hour$min$sec.BHZ.sac
  mv *.HHN.*.$year.$julday.$hour$min$sec.SAC $outdir/$stn.$year$julday$hour$min$sec.BHN.sac
  mv *.HHE.*.$year.$julday.$hour$min$sec.SAC $outdir/$stn.$year$julday$hour$min$sec.BHE.sac
